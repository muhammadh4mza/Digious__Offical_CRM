...
const AdminSidebar = ({ isCollapsed, setIsCollapsed, activeItem, setActiveItem }) => {
  ...
  const menuItems = [
    ...
    { id: 'sales', icon: TrendingUp, label: 'Sales', path: '/admin/sales' },
        ...
        <div className="p-4 border-b border-blue-200/40">
          {!isCollapsed ? (
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 bg-gradient-to-br from-red-500 to-pink-600 rounded-xl flex items-center justify-center text-white font-semibold text-lg shadow-lg shadow-red-500/30">
                {String(user?.name || user?.username || user?.email || 'Admin').charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-800 truncate">{user?.name || user?.email || 'Admin'}</p>
